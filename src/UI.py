...
def plotting(matriksOfPoint, n, result):
    if (len(matriksOfPoint[0])) == 3:
        p = str(input("It seems like the points are built on 3D, do you want to plot the points? (y/n)\n  > "))
        if p == "y":
...
def program():
    ...
    print("Closest pair using Bruteforce algorithm found! Here's the result:")
    ...
    print("Closest pair using Divide and Conquer algorithm found! Here's the result:")
    ...
    print("Above results are executed using your PC specification below:")
    getPCSpec()
